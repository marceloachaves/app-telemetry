import { Body, Controller, Get, Param, Post, Req, UnprocessableEntityException } from '@nestjs/common';
...
export class TelemetryController {
  ...
  async create(
    ...
    if (!req.user) {
      throw new UnprocessableEntityException('User information is missing from request');
    }
    ...
    return await createTelemetryUsecase.execute(TelemetryMapper.fromCreateTelemetryDto(createTelemetryDto));
  }

  @Get(':deviceId')
  async findByDevice(@Param('deviceId') deviceId: string, @Req() req: requestUserInterface.RequestWithUser) {
    if (!req.user) {
      throw new UnprocessableEntityException('User information is missing from request');
    }
    const getTelemetryUsecase = new GetTelemetryUsecase(this.telemetryRepository, this.devicesRepository, req.user);
